       ...
       <AnimatePresence mode="wait">
         {activeTab === 'home' ? (
           ...
           >
              ...
              <section className="relative overflow-hidden rounded-[2.5rem] bg-white border border-[#D2D2D7]/30 shadow-sm">
                 <div className="absolute top-0 right-0 p-8">
                   <div className="bg-blue-50 px-3 py-1 rounded-full border border-blue-100 flex items-center gap-2">
                     ...
                     <span className="text-[10px] font-black uppercase tracking-widest text-blue-700">Centro Clienti</span>
                   </div>
                 </div>
              ...
              {/* Slogan Card */}
              <div className="p-12 bg-blue-600 rounded-[2.5rem] text-white flex flex-col md:flex-row items-center gap-8 relative overflow-hidden">
                <div className="relative z-10 space-y-4">
                   <h3 className="text-4xl font-black tracking-tighter leading-none">Professionalità garantita,<br />in un solo tocco.</h3>
                   <p className="text-white/70 font-bold max-w-sm text-sm">Ogni professionista è verificato manualmente dal nostro supporto.</p>
                   <Button 
                    variant="outline" 
              ...
              <div className="space-y-6">
                <div className="flex items-center justify-between px-2">
                  <h2 className="text-2xl font-black tracking-tight text-[#1D1D1F]">Attività Recenti</h2>
                  <Button 
                    variant="ghost" 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {jobs.slice(0, 2).map((job) => (
                    <Card key={job.id} className="rounded-[2rem] border-[#D2D2D7]/30 p-6 bg-white flex flex-col justify-between">
                       <div>
                         <div className="flex items-center justify-between mb-2">
                            <span className="text-[9px] font-black uppercase tracking-widest text-blue-600">{job.category}</span>
                            <span className="text-[9px] font-black uppercase tracking-widest text-gray-400">{job.createdAt?.seconds ? new Date(job.createdAt.seconds * 1000).toLocaleDateString() : 'Oggi'}</span>
                         </div>
                         <h4 className="font-black text-lg mb-1">{job.title}</h4>
                         <p className="text-xs font-bold text-[#86868B] line-clamp-1">{job.description}</p>
                       </div>
                       <div className="mt-6 flex items-center justify-between border-t border-gray-100 pt-4">
                          <span className={cn("text-[9px] font-black uppercase px-2 py-1 rounded-md", 
                            job.status === 'open' ? 'bg-green-50 text-green-600' : 'bg-blue-50 text-blue-600'
                          )}>{job.status}</span>
                          <span className="font-black text-sm">{job.proposalCount || 0} Proposte</span>
                       </div>
                    </Card>
                  ))}
                  {jobs.length === 0 && (
                     <div className="md:col-span-2 py-10 bg-gray-50 rounded-[2rem] border border-dashed border-gray-200 text-center">
                        <p className="text-xs font-black text-gray-400 uppercase tracking-widest">Nessuna attività recente</p>
                     </div>
                  )}
                </div>
              </div>
           ...
           >
              <div className="flex items-center justify-between px-2">
                <div>
                  <h2 className="text-4xl font-black tracking-tight text-[#1D1D1F]">Gestione Richieste</h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {jobs.map((job) => (
                  ...
                  >
                    <Card 
                      className="rounded-[2rem] bg-white border border-[#D2D2D7]/30 hover:shadow-xl hover:shadow-black/5 transition-all cursor-pointer group overflow-hidden"
                      onClick={async () => {
                    ...
                    >
                      <CardHeader className="flex-row items-start justify-between space-y-0 pb-2">
                        <div className="space-y-1">
                          <span className="text-[10px] font-black uppercase tracking-widest text-[#86868B] bg-[#F5F5F7] px-2 py-0.5 rounded">
                            {job.category}
                          </span>
                          <CardTitle className="text-xl font-black tracking-tight group-hover:text-blue-600 transition-colors flex items-center justify-between">
                            {job.title}
                            {job.hasNewProposals && (
                              <span className="flex h-2.5 w-2.5 rounded-full bg-orange-500 animate-pulse shrink-0 ml-2 shadow-[0_0_10px_rgba(249,115,22,0.5)]" />
                            )}
                          </CardTitle>
                        </div>
                      ...
                      <CardContent className="space-y-4">
                        <p className="text-sm text-[#86868B] font-bold line-clamp-2 leading-relaxed">
                          {job.description}
                        </p>
                        ...
                        <div className="pt-4 border-t border-[#D2D2D7]/30 flex flex-col gap-4">
                          <div className="flex items-center justify-between">
                            <div className="flex -space-x-2">
                               {[...Array(Math.min(3, job.proposalCount || 0))].map((_, i) => (
                                 <div key={i} className="w-8 h-8 rounded-full border-2 border-white bg-[#F5F5F7] flex items-center justify-center text-[10px] font-black text-[#86868B]">
                                   P
                                 </div>
                               ))}
                            </div>
                          ...
                          {job.status === 'open' && (
                            <div className="flex flex-col sm:flex-row gap-3 w-full">
                              <Button 
                                ...
                                className="flex-1 h-11 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-black text-[11px] uppercase tracking-widest shadow-lg shadow-blue-600/20 transition-all border-none"
                              >
                                <Clock className="w-4 h-4 mr-2" />
                                Prolunga (+30gg) • 1 Token
                              </Button>
                              <Button 
                                variant="outline"
                                onClick={(e) => {
                                ...
                                className="flex-1 h-11 rounded-xl bg-red-50 text-red-600 border-red-100 font-black text-[11px] uppercase tracking-widest hover:bg-red-100 transition-all"
                              >
                                Elimina Richiesta
                              </Button>
                            </div>
                          ...
                          {job.status === 'completed' && !job.reviewId && (
                            <Button 
                              ...
                              className="w-full h-11 rounded-xl bg-orange-500 hover:bg-orange-600 text-white font-black text-[11px] uppercase tracking-widest shadow-lg shadow-orange-500/20 transition-all border-none flex items-center gap-2"
                            >
                              <Star className="w-4 h-4" /> Lascia una Recensione (Importante!)
                            </Button>
                          )}
                        </div>
                      </CardContent>
